...
Тогда команда init будет выполняться слудующим образом:
...
Сам сайт размещается в директории ```site```, которая создается в той же директории где выполняется команда

Шаги установки:

1. Выбрать версию php
2. Выбрать версию mysql - 5.7 или 8.0
3. Устанавливать ли node.js - Y или N, если установите Y, то будет создан сервис с node js 20 версии
