...
При запуске контейнеров app и nignx запускается скрипт - _docker/bin/create_simlink.sh, он создает ссылки внутри контейнеров.

Ссылки берутся из файла - _docker/files/simlinks, смотрите simlinks.example для того, чтобы понимать структуру файла.
...
Если же вам нужно дополнительные ссылки добавить, то добавляете по аналогии с simlinks.example, все пути относительно структуры контейнера. После этого необходимо выполнить команду:
