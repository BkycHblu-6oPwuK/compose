...
Версию mysql можно менять в файле docker-compose. Для этого найдите сервися mysql и его ключ image.
...
Вы можете изменить версию ``` node ```  и коренвую директории поменяв значение переменных ``` NODE_VERSION ``` и ``` NODE_PATH ```
