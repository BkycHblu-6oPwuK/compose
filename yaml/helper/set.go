...
func buildNginxService() service.Service {
	nginxService := service.NewServiceBuilder().
		...
		AddVolume("${" + config.DockerPathVarName + "}/" + Nginx + "/conf.d:/etc/nginx/conf.d").
		AddPort("80:80").
		AddPort("443:443").
		AddDependency(App).
		AddDefaultNetwork().
...
func buildAppService(yamlConfig *config.YamlConfig) service.Service {
	...
	appService := service.NewServiceBuilder().
		...
		AddVolume("${"+config.SitePathVarName+"}:"+config.SitePathInContainer).
		AddVolume("${"+config.DockerPathVarName+"}/"+App+"/php-"+phpVersionVarName+"/php.ini:/usr/local/etc/php/conf.d/php.ini").
		AddVolume("${"+config.DockerPathVarName+"}/"+App+"/php-"+phpVersionVarName+"/xdebug.ini:/usr/local/etc/php/conf.d/xdebug.ini").
		AddVolume("${"+config.DockerPathVarName+"}/"+App+"/php-fpm.conf:/usr/local/etc/php-fpm.d/zzzzwww.conf").
		AddVolume("${"+config.DockerPathVarName+"}/"+App+"/nginx:/etc/nginx/conf.d").
		AddPort("9000:9000").
		AddPort("6001:6001").
...
func buildSphinxService() service.Service {
	sphinxService := service.NewServiceBuilder().
		...
		AddPort("9312:9312").
		AddPort("9306:9306").
		AddVolume("${" + config.DockerPathVarName + "}/" + Sphinx + "/sphinx.conf:/usr/local/etc/sphinx.conf").
